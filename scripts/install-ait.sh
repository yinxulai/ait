...
case "${OS}" in
  ...
  Darwin)
    ...
    ;;
  *)
    echo "[AIT] 暂不支持的操作系统: ${OS}" >&2
    echo "[AIT] 支持的系统: Linux, macOS (Darwin)" >&2
    exit 1
    ;;
esac
...
case "${ARCH}" in
  x86_64|amd64)
    FILE_NAME="ait-${OS_TYPE}-amd64"
    ;;
  aarch64|arm64)
    FILE_NAME="ait-${OS_TYPE}-arm64"
    ;;
  armv7l|arm)
    if [[ "${OS_TYPE}" == "darwin" ]]; then
      echo "[AIT] macOS 不支持 ARM32 架构" >&2
      exit 1
    fi
    FILE_NAME="ait-${OS_TYPE}-arm"
    ;;
  i386|i686)
    if [[ "${OS_TYPE}" == "darwin" ]]; then
      echo "[AIT] macOS 不支持 32 位架构" >&2
      exit 1
    fi
    FILE_NAME="ait-${OS_TYPE}-386"
    ;;
  *)
