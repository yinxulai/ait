...
func TestJSONRenderer_Render_SingleModel(t *testing.T) {
	...
	modelData, ok := models[0].(map[string]interface{})
	if !ok {
		t.Fatal("Expected model data to be a map")
	}

	if modelData["is_thinking"] != true {
		t.Errorf("Expected is_thinking true, got %v", modelData["is_thinking"])
	}

	if modelData["avg_thinking_token_count"] != float64(120) {
		t.Errorf("Expected avg_thinking_token_count 120, got %v", modelData["avg_thinking_token_count"])
	}

	if modelData["min_thinking_token_count"] != float64(100) {
		t.Errorf("Expected min_thinking_token_count 100, got %v", modelData["min_thinking_token_count"])
	}

	if modelData["max_thinking_token_count"] != float64(140) {
		t.Errorf("Expected max_thinking_token_count 140, got %v", modelData["max_thinking_token_count"])
	}
...
func createTestReportDataForJSON() types.ReportData {
	...
	data.AvgThinkingTokenCount = 120
	data.MinThinkingTokenCount = 100
	data.MaxThinkingTokenCount = 140
	data.AvgTPS = 300.0
	data.MinTPS = 250.0
	data.MaxTPS = 350.0
